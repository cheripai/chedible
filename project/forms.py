...
class AddRestaurantForm(Form):
    ...
    category = StringField('Category', validators=[DataRequired(message="A category is required"), Length(min=2, max=32, message="Must be between 2 and 32 characters"), Regexp('(^([A-Za-z0-9_.\' ]*\,)*[A-Za-z0-9 ]*$)', message="Must be a comma separated list")])
    ...
    tags = StringField('Tags', validators=[Optional(), Length(min=2, max=64, message="Tags must be between 2 and 64 characters"), Regexp('(^([A-Za-z0-9_.\' ]*\,)*[A-Za-z0-9 ]*$)', message="Must be a comma separated list ")])
...
class AddDishForm(Form):
    ...
    price = StringField('Price', validators=[Optional(), Regexp('(^[0-9]*\.[0-9][0-9])$', message="Invalid price format. Must be a number followed by 2 decimal places. Example: 12.34")])
